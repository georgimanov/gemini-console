...
function isRetryable(error: unknown): boolean {
  ...
  return typeof status === "number" && Defaults.RETRY.RETRYABLE_STATUS_CODES.has(status);
}
 ...
 * Every provider SDK in use here (Google, OpenAI) surfaces a `.status`
 * field on its error class, so this works unchanged across providers.
 */
export async function withRetry<T>(fn: () => Promise<T>): Promise<T> {
  let lastError: unknown;
  const { MAX_ATTEMPTS, BASE_DELAY_MS } = Defaults.RETRY;
